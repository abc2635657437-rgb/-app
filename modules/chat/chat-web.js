(function () {
  ...
  window.openTravelUser=async userId=>{
    if(!auth().requireLogin())return;
    const version=epoch;
    try {const user=await auth().request('/api/dm/users/'+encodeURIComponent(userId));if(version!==epoch)return;document.getElementById('dmUser')?.remove();const node=document.createElement('div');node.id='dmUser';node.className='dm-overlay';node.innerHTML='<div class="dm-profile"><button class="btn alt small" id="dmProfileBack">返回</button><div style="margin-top:18px">'+avatar(user)+'</div><h2>'+esc(name(user))+'</h2><p>'+esc(user.bio||'这位旅行者还没有填写简介。')+'</p>'+(user.id!==uid()?'<button class="btn" id="dmStart">私聊</button>':'<p class="muted">这是你的主页</p>')+'</div>';document.body.appendChild(node);node.querySelector('#dmProfileBack').onclick=()=>node.remove();if(node.querySelector('#dmStart'))node.querySelector('#dmStart').onclick=()=>window.openDirectChat(user.id);}
