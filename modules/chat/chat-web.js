(function () {
  ...
  window.openTravelUser=async userId=>{
    ...
    try {
      ...
      const user=result.profile,stats=result.stats||{},posts=result.posts||[],routes=result.routes||[];document.getElementById('dmUser')?.remove();
      const node=document.createElement('div');node.id='dmUser';node.className='dm-overlay';
      node.innerHTML='<div class="dm-profile" role="dialog" aria-label="'+esc(name(user))+' 的公开主页"><button class="btn alt small" id="dmProfileBack">返回</button><div class="dm-profile-hero">'+avatar(user)+'<div><h2>'+esc(name(user))+'</h2><p class="muted">'+esc(user.bio||'这位旅行者还没有填写简介。')+'</p></div></div><div class="dm-profile-stats"><span><b>'+Number(stats.posts||0)+'</b><small>发布</small></span><span><b>'+Number(stats.routes||0)+'</b><small>路线</small></span><span><b>'+Number(stats.followers||0)+'</b><small>粉丝</small></span><span><b>'+Number(stats.likes||0)+'</b><small>获赞</small></span></div><div class="dm-profile-actions">'+(result.isSelf?'<button class="btn" id="dmOwnProfile">编辑我的主页</button>':'<button class="btn alt" id="dmFollow">'+(result.isFollowing?'已关注':'关注')+'</button><button class="btn" id="dmStart">私聊</button>')+'</div><h3>公开路线</h3>'+(routes.length?routes.map(route=>TravelPostUI.routeCard(route)).join(''):'<p class="muted">还没有公开路线。</p>')+'<h3>最近发布</h3>'+(posts.length?posts.slice(0,8).map(post=>TravelPostUI.card(post,{showFollow:false})).join(''):'<p class="muted">还没有公开发布。</p>')+'</div>';
