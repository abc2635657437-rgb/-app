(function () {
  ...
  function render() {
    ...
    const bio = profile?.bio || (loggedIn ? '还没有填写简介' : '保存路线，记录旅行。');
    setPage('<div class="eyebrow">MY SPACE</div><div class="profile-hero"><button class="profile-avatar-action" onclick="' + (loggedIn ? 'editProfileAvatar()' : 'TravelWorldAuth.showLogin()') + '" aria-label="更换头像"><img src="' + safe(profile?.avatar_url || fallback) + '" class="profile-large-avatar"><span>更换</span></button><div><h1>' + safe(name) + '</h1><button class="profile-bio-action" onclick="' + (loggedIn ? 'editProfileBio()' : 'TravelWorldAuth.showLogin()') + '">' + safe(bio) + '</button><button class="profile-text-action" onclick="' + (loggedIn ? 'editTravelProfile()' : 'TravelWorldAuth.showLogin()') + '">' + (loggedIn ? '编辑昵称' : '登录 / 注册') + '</button></div></div>' +
      ...
      '<div class="profile-settings card"><h3>设置</h3><details open><summary>账号与个人资料</summary><button onclick="editTravelProfile()">昵称<span>›</span></button><button onclick="openSetting(\'account\')">账号与安全<span>›</span></button><button onclick="openSetting(\'privacy\')">隐私设置<span>›</span></button><button onclick="openSetting(\'blocked\')">黑名单<span>›</span></button></details><details><summary>体验偏好</summary><button onclick="toggleProfileLanguage()">语言<span>' + (state().lang === 'en' ? 'English' : '简体中文') + ' ›</span></button><button onclick="openSetting(\'notifications\')">通知<span>›</span></button><button onclick="openSetting(\'messages\')">消息<span>›</span></button><button onclick="openSetting(\'content_preferences\')">内容偏好<span>›</span></button><button onclick="openSetting(\'general\')">通用设置<span>›</span></button><button onclick="changeProfileCurrency()">默认货币<span>' + (state().currency || 'CNY') + ' ›</span></button><button onclick="clearProfileCache()">清除缓存<span>›</span></button></details><details><summary>关于 Travel World</summary><button onclick="openSetting(\'about\')">关于我们<span>›</span></button><button onclick="openSetting(\'terms\')">用户协议<span>›</span></button><button onclick="openSetting(\'policy\')">隐私政策<span>›</span></button></details>' + (loggedIn ? '<button class="danger" onclick="logoutProfile()">退出登录<span>›</span></button>' : '') + '</div>');
  }
  ...
  const post = (item, type) => window.TravelPostUI.card(item,{ownerActions:type==='posts',showFollow:false}) + (type==='favorites'?'<button class="btn alt small" onclick="removeProfileFavorite(\''+item.id+'\')">取消收藏</button>':'');
  ...
  window.openProfilePanel = async type => {
    ...
    try {
      ...
      if (type === 'favorites') body = items.map(item => post(item, type)).join('') + favoriteRoutes.map(item => window.TravelPostUI.routeCard(item)+'<button class="btn alt small" onclick="removeProfileRouteFavorite(\'' + item.id + '\')">取消收藏</button>').join('');
      if (type === 'following') body = items.map(item => person(item, '<div class="profile-row-actions"><button onclick="openDirectChat(\'' + item.id + '\')">私聊</button><button onclick="toggleProfileFollow(\'' + item.id + '\',\'following\')">取消关注</button><button onclick="blockProfile(\'' + item.id + '\')">屏蔽</button></div>')).join('');
      if (type === 'followers') body = items.map(item => person(item, '<div class="profile-row-actions"><button onclick="openDirectChat(\'' + item.id + '\')">私聊</button><button onclick="toggleProfileFollow(\'' + item.id + '\',\'followers\')">关注</button><button onclick="blockProfile(\'' + item.id + '\')">屏蔽</button></div>')).join('');
      if (type === 'routes') body = items.map(item => window.TravelPostUI.routeCard(item)+'<div class="profile-item-actions"><button onclick="openRouteEditor(\''+item.id+'\')">编辑</button><button onclick="publishProfileRoute(\'' + item.id + '\',' + (!item.is_public) + ')">' + (item.is_public ? '取消公开' : '公开路线') + '</button><button onclick="deleteProfileRoute(\'' + item.id + '\')">删除</button></div>').join('');
  ...
  window.editProfilePost = async id => { const item=window.TravelPostUI.get(id)||{}; const title = prompt('标题（可留空）', item.title||''); if (title === null) return; const content = prompt('正文（可留空）', item.content||''); if (content === null) return; try { await auth().request('/api/community/posts/' + id, { method: 'PATCH', body: JSON.stringify({ title: title.trim(), content: content.trim() }) }); await openProfilePanel('posts'); window.dispatchEvent(new Event('tw-auth-change')); } catch (error) { alert(error.message); } };
  ...
  window.editTravelProfile = async () => { if (!auth().requireLogin()) return; const name = prompt('昵称', profile?.display_name || ''); if (name === null || !name.trim()) return; try { profile = await auth().request('/api/users/me', { method: 'PATCH', body: JSON.stringify({ display_name: name.trim() }) }); render(); } catch (error) { alert(error.message); } };
  window.editProfileBio = async () => { if (!auth().requireLogin()) return; const bio = prompt('个人简介', profile?.bio || ''); if (bio === null) return; try { profile = await auth().request('/api/users/me', { method: 'PATCH', body: JSON.stringify({ bio: bio.trim() }) }); render(); } catch (error) { alert(error.message); } };
  window.editProfileAvatar = () => { if (!auth().requireLogin()) return; const picker = document.createElement('input'); picker.type = 'file'; picker.accept = 'image/*'; picker.onchange = async () => { if (!picker.files[0]) return; const form = new FormData(); form.append('file', picker.files[0]); try { profile = await auth().request('/api/users/me/avatar', { method: 'POST', body: form }); render(); } catch (error) { alert(error.message); } }; picker.click(); };
