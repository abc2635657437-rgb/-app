(function () {
  ...
  async function followMapUser(userId, button) { const auth = window.TravelWorldAuth; if (!auth?.requireLogin() || button?.disabled) return; const before = button?.textContent || '关注'; if (button) { button.disabled = true; button.textContent = before === '关注' ? '已关注' : '关注'; } try { const result = await auth.request('/api/users/' + encodeURIComponent(userId) + '/follow', { method: 'POST' }); if (button) button.textContent = result.following ? '已关注' : '关注'; } catch (error) { if (button) button.textContent = before; status('操作失败，请重试'); } finally { if (button) button.disabled = false; } }
  async function openMapDirectChat(userId) { const auth = window.TravelWorldAuth; if (!auth?.requireLogin()) return; try { const result = await auth.request('/api/direct-chats/' + encodeURIComponent(userId), { method: 'POST', body: '{}' }); if (window.openBuddyChat) await window.openBuddyChat(result.chatId, encodeURIComponent(result.user?.display_name || '旅行者')); else if (window.go) window.go('friends'); status('私聊已打开。'); } catch (error) { status(error.message); } }
  async function loadOnlineUsers() { try { const country = document.getElementById('mapOnlineCountry')?.value.trim() || ''; const response = await fetch('/api/map/online-users' + (country ? '?country=' + encodeURIComponent(country) : '')); const payload = await response.json(); if (!response.ok) throw new Error(payload.error || '无法读取在线用户'); if (!map || !window.L) return; clearOnlineMarkers(); payload.users.forEach(entry => { const profile = entry.profiles || {}; const name = String(profile.display_name || profile.username || '旅行者').replace(/[<>]/g, ''); const location = String(entry.city || entry.country || '公开位置').replace(/[<>]/g, ''); const userId = String(profile.id || entry.user_id || ''); const icon = L.divIcon({ className: 'tw-q-pin', html: '<div class="tw-q-pin__body"><span class="tw-q-pin__icon">🧭</span><span class="tw-q-pin__label">' + name + '</span></div>', iconSize: [36, 36], iconAnchor: [18, 18] }); const popup = '<strong>' + name + '</strong><br>' + location + (profile.bio ? '<br><span>' + String(profile.bio).replace(/[<>]/g, '') + '</span>' : '') + '<br><button onclick="window.followMapUser(\'' + userId + '\',this)">关注</button> <button onclick="window.openMapDirectChat(\'' + userId + '\')">私聊</button>'; onlineMarkers.push(L.marker([entry.latitude, entry.longitude], { icon }).addTo(map).bindPopup(popup)); }); status((country ? country + ' · ' : '') + '已显示 ' + payload.users.length + ' 位主动共享位置的在线用户。'); } catch (error) { status('在线用户暂时不可用：' + error.message); } }
  ...
  window.followMapUser = followMapUser;
  window.openMapDirectChat = openMapDirectChat;
