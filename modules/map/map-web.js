(function () {
  ...
  async function publishPresence() { const auth = window.TravelWorldAuth; if (!auth?.requireLogin()) return; if (!lastPosition) return locateUser(); try { await auth.request('/api/map/presence', { method: 'POST', body: JSON.stringify({ latitude: lastPosition.coords.latitude, longitude: lastPosition.coords.longitude, sharing_enabled: true }) }); status('位置共享已开启，5 分钟未更新将自动下线。'); } catch (error) { document.getElementById('mapShareLocation').checked = false; status('位置共享未开启：' + error.message); } }
