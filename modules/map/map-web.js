(function () {
  ...
  async function openOnlineUser(userId) { const auth = window.TravelWorldAuth; if (!auth?.requireLogin()) return; try { const result = await auth.request('/api/direct-chats/' + encodeURIComponent(userId), { method: 'POST' }); if (window.openBuddyChat) return window.openBuddyChat(result.chatId, encodeURIComponent(result.user?.display_name || result.user?.username || '旅行者')); status('私聊已建立，请在旅友页面继续聊天。'); if (window.go) window.go('friends'); } catch (error) { status('无法发起私聊，请重试'); } }
