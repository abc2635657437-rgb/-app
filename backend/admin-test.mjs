...
try{
  ...
  await call('/api/admin/me',tokens[1],'GET',null,403);await db.from('admin_users').insert({user_id:ids[0],role:'content_moderator'});
  ...
  await call(`/api/admin/posts/${post.id}`,tokens[1],'DELETE',null,403);await call(`/api/admin/posts/${post.id}`,tokens[0],'DELETE',null,204);await call(`/api/admin/posts/${post.id}`,tokens[0],'DELETE',null,404);await call(`/api/admin/users/${ids[1]}/status`,tokens[0],'PATCH',{status:'suspended'},404);
  const {data:audit}=await db.from('admin_audit_log').select('action,result').eq('admin_id',ids[0]).eq('target_id',post.id).single();assert.deepEqual(audit,{action:'delete_post',result:'success'});
  console.log(JSON.stringify({ok:true,checks:['public-dashboard','real-post-data','admin-delete-post','delete-audit','rbac-private-apis']}));
