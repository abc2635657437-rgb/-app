...
try {
  ...
  const routeCreated = await api('/api/routes', a, { method: 'POST', body: JSON.stringify({ title: '西湖真实路线', destination: '杭州', transport_mode: 'walking', distance_meters: 3200, duration_seconds: 4200, geometry: { type: 'LineString', coordinates: [[120.13,30.25],[120.15,30.24]] }, points: [{ id: 'west-lake', name: '西湖', address: '杭州市西湖区', latitude: 30.25, longitude: 120.13 }, { id: 'leifeng', name: '雷峰塔', address: '杭州市西湖区南山路', latitude: 30.24, longitude: 120.15 }] }) });
  const routeId = routeCreated.id; const routeDetail = await api(`/api/routes/${routeId}`, a); if (routeDetail.trip_days?.[0]?.trip_places?.length !== 2 || routeDetail.transport_mode !== 'walking') throw new Error('结构化路线保存失败');
  await api(`/api/routes/${routeId}`, a, { method: 'PATCH', body: JSON.stringify({ title: '西湖更新路线', is_public: true }) });
  const routePost = await api('/api/community/posts', a, { method: 'POST', body: JSON.stringify({ title: '', content: '沿湖慢行记录', type: 'story', route_id: routeId }) });
  const postDetail = await api(`/api/community/posts/${routePost.id}`, b); if (postDetail.route?.id !== routeId || postDetail.route?.trip_days?.[0]?.trip_places?.length !== 2) throw new Error('帖子路线绑定失败');
  await api(`/api/routes/${routeId}/like`, b, { method: 'POST' }); await api(`/api/routes/${routeId}/favorite`, b, { method: 'POST' }); await api(`/api/routes/${routeId}/comments`, b, { method: 'POST', body: JSON.stringify({ content: '路线很好' }) });
  ...
  const disposableRoute = await api('/api/routes', a, { method: 'POST', body: JSON.stringify({ title: '可独立删除路线', points: [{ name: '起点', latitude: 30.24, longitude: 120.14 }, { name: '终点', latitude: 30.25, longitude: 120.16 }] }) });
  const retainedPost = await api('/api/community/posts', a, { method: 'POST', body: JSON.stringify({ content: '路线删除后仍应保留的记录', route_id: disposableRoute.id }) });
  await api(`/api/routes/${disposableRoute.id}`, a, { method: 'DELETE' }); const retainedDetail = await api(`/api/community/posts/${retainedPost.id}`, a); if (retainedDetail.route_id || retainedDetail.route) throw new Error('路线删除错误地删除了帖子');
  ...
  console.log(JSON.stringify({ ok: true, checks: ['auth', 'profile', 'public-profile', 'avatar', 'stats', 'post', 'post-detail', 'post-route-binding', 'post-idempotency', 'post-edit', 'media', 'like', 'comment', 'comment-delete', 'favorite', 'follow', 'block', 'unblock', 'my-posts', 'favorites-list', 'following-list', 'followers-list', 'interactions', 'settings', 'routes-list', 'route-create', 'route-detail', 'route-edit', 'route-delete-independent', 'route-publish', 'route-like', 'route-comment', 'route-favorite', 'trip', 'trip-edit', 'application', 'relation', 'chat', 'notifications'] }));
} finally {
