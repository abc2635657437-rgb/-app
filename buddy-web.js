(function () {
  ...
  function appCard(item, received) {
    ...
    return '<div class="card"><div class="row"><b>' + safe(item.trip?.title || '旅行申请') + '</b><span class="pill">' + safe(statusText[item.status] || item.status) + '</span></div><div class="muted">' + (received ? '申请人：' + safe(item.applicant?.display_name || '旅行者') : safe(item.trip?.destination || '')) + '</div><div class="muted" style="margin:7px 0">' + safe(item.message) + '</div>' + actions + '</div>';
  }
  function render() {
